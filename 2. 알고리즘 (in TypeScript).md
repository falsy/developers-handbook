...
> 보통은 내부적으로 `최소 힙`을 사용하지만, 코드의 단순화를 위해 `배열`를 사용하였습니다.
...
> 보통은 내부적으로 연결된 간선을 `최소 힙`을 사용하지만, 코드의 단순화를 위해 `배열`를 사용하였습니다.
...
보통 분리 집합 자료구조(Disjoint-set, Union-Find)를 사용해서 간선을 하나씩 추가할 때 사이클을 효율적으로 확인합니다.
